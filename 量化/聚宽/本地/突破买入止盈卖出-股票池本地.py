...
def should_trade(context, current_price, past_4_days_high):
    ...
    if not has_position:  # 没有持仓时寻找买入机会
        ...
        if current_price >= past_4_days_high:
            log.info(f"股票代码： {context.security} ; 当前价格 {current_price} 大于等于过去4日收盘价最高点 {past_4_days_high}，准备买入")
            signal = "BUY"
        ...
        if current_price <= stop_profit_price and high_hist_price/avg_cost > 1.05:
            log.info(f"股票代码： {context.security} ; 触发止盈条件： 最高点位：{high_hist_price} ,止盈点： {stop_profit_price} ,当前价为： {current_price} , 准备卖出")
        ...
        if current_price <= stop_loss_price:
            log.info(f"股票代码： {context.security} ; 触发止损条件： 成本价：{avg_cost} ,止损点： {stop_loss_price} ,当前价为： {current_price} , 准备卖出")
            signal = "SELL"
...
def execute_trade(context, signal, current_price, past_4_days_high):
    ...
    if signal == "BUY":
        ...
        if amount > 0:
            ...
            log.info(f"买入 {context.security} ; {qty} 手，股数: {amount} , 买入总价 {total_amount}, 买入价 {current_price}，过去4日最高收盘价: {past_4_days_high}")
            signal = None
        ...
        log.info(
            f"卖出 {context.security};  {position.total_amount} 股，价格: {current_price} ，最高价: {context.highest_price}")
