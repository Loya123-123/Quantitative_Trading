...
def get_price_data(ContextInfo):
    ...
    try:
        ...
        ContextInfo.current_date = bar_date
        log_info(f"  [价格数据] 获取截止时间: {ContextInfo.current_date}")
        ...
        if 'time' in history_df.columns:
            history_df = history_df.copy()  # 避免修改原始数据
...
def get_account_info(ContextInfo):
    ...
    try:
        ...
        ContextInfo.short_position = 0  # 重置空头持仓状态
        # 重置开仓日期
        ContextInfo.long_open_date = None  # 重置多头开仓日期
        ContextInfo.short_open_date = None  # 重置空头开仓日期
            ...
            for pos in position_details:
                ...
                if symbol == ContextInfo.stock_code:
                    ...
                    if position_type == 48:  # 多头持仓
                        ...
                        ContextInfo.long_open_date = pos.m_strOpenDate  # 多头开仓日期
                        ...
                        ContextInfo.short_open_date = pos.m_strOpenDate  # 空头开仓日期
...
def generate_signal(ContextInfo, price_data):
    ...
    try:
        ...
        lower_channel = price_data['close'].iloc[-ContextInfo.entry_window - 1:-1].min()

        # 计算离市信号 - 离市通道
        # 做多止盈 过去exit_window天收盘价的最低价
        exit_lower = price_data['close'].iloc[-ContextInfo.exit_window - 1:-1].min()
        ...
        if ContextInfo.long_position == 0:  # 无多头持仓，判断是否做多
            # 买入1（做多）：当日价格 > 前10日收盘价最高点时，即当前价格突破前10日高点，当日立刻执行做多
        ...
        elif ContextInfo.long_position == 1:  # 有多头持仓，判断是否平多
            log_info("  [信号生成] 当前持有多头仓位，判断是否平仓")

            log_info(f"  [信号生成] 多头止盈价格计算:")
            stop_profit_price = ContextInfo.highest_after_entry - (
                    ContextInfo.highest_after_entry - ContextInfo.long_entry_price) * ContextInfo.stop_profit_ratio
            log_info(f"    公式: 最高价 - (最高价 - 入场价) * 止盈比例")
            log_info(
                f"    数值: {ContextInfo.highest_after_entry} - ({ContextInfo.highest_after_entry} - {ContextInfo.long_entry_price}) * {ContextInfo.stop_profit_ratio} = {stop_profit_price:.4f}")
            # 止盈信号1 - 价格跌破止盈做多点且价格小于最高价回撤一定比例

            # 止损信号2 买入价 - 2 * ATR
            stop_loss_price = ContextInfo.long_entry_price - ContextInfo.stop_loss_multiplier * ContextInfo.N
            ...
            # 止损信号 - 价格下跌超过2ATR
                ...
                log_info(f"    入场价: {ContextInfo.long_entry_price}, ATR: {ContextInfo.N:.4f}")
                return (-1, 1)  # 平仓信号，多头操作
            else:
                log_info("  [信号生成] 无平多信号")

        # 判断空头信号（无空头持仓时可做空，有空头持仓时判断是否平空）
        if ContextInfo.short_position == 0:  # 无空头持仓，判断是否做空
        ...
        elif ContextInfo.short_position == 1:  # 有空头持仓，判断是否平空
            ...
            stop_profit_price = ContextInfo.lowest_after_entry + (
                    ContextInfo.short_entry_price - ContextInfo.lowest_after_entry) * ContextInfo.stop_profit_ratio
            log_info(f"  [信号生成] 空头止盈价格计算:")
            log_info(f"    公式: 最低价 + (入场价 - 最低价) * 止盈比例")
            log_info(
                f"    数值: {ContextInfo.lowest_after_entry} + ({ContextInfo.short_entry_price} - {ContextInfo.lowest_after_entry}) * {ContextInfo.stop_profit_ratio} = {stop_profit_price:.4f}")

            # 空仓止损信号3 卖出价 + 2 * ATR
            stop_loss_price = ContextInfo.short_entry_price + ContextInfo.stop_loss_multiplier * ContextInfo.N
            ...
            # 止损信号 - 价格上涨超过2ATR
            elif current_price > ContextInfo.short_entry_price + ContextInfo.stop_loss_multiplier * ContextInfo.N:
                log_info("  [信号生成] 产生空头止损信号：价格上涨超过2ATR")
                log_info(f"    当前价格: {current_price} > 止损价格: {stop_loss_price:.4f}")
                log_info(f"    入场价: {ContextInfo.short_entry_price}, ATR: {ContextInfo.N:.4f}")
                return (-1, -1)  # 平仓信号，空头操作
            else:
                log_info("  [信号生成] 无平空信号")
