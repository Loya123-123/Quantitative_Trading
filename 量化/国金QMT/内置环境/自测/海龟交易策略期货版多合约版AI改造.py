...
如果是开仓信号且持仓不满4个 或 合约距离到期大于30天 则才能开仓。
...
def handlebar(ContextInfo):
    ...
    for stock_code in g.current_trading_contracts:
        ...
        try:
            ...
            if g.position_count >= 4:
                log_info("[交易执行] 当前持仓已满4个，不执行新的开仓操作")
            else:
                log_info("[交易执行] 当前持仓不满4个，可以执行新的开仓操作")
    ...
    try:
        ...
        if position_details:
            ...
            for pos in position_details:
                # 获取 pos 对象转json信息
                log_info(f"  [账户信息] 获取持仓详情: {pos.__dict__}")
...
def generate_signal(ContextInfo, price_data):
    ...
    try:
        ...
        if g.long_position[g.current_stock_code] == 0 and g.expire_date_diff[
            g.current_stock_code] > 30 and g.position_count < 4:  # 无多头持仓，判断是否做多 距离到期日大于30天 持仓小于4个
            # 买入1（做多）：当日价格 > 前10日收盘价最高点时，即当前价格突破前10日高点，当日立刻执行做多
        ...
        if g.short_position[g.current_stock_code] == 0 and g.expire_date_diff[
            g.current_stock_code] > 30 and g.position_count < 4:  # 无空头持仓，判断是否做空 且 距离到期日大于30天 持仓小于4个
    ...
    try:
        ...
        if signal_type > 0:  # 开仓信号
            if position_type > 0:  # 做多
                ...
                if g.long_position[g.current_stock_code] == 0:
                    ...
                    order_info = passorder(0, 1101, ContextInfo.account_id, g.current_stock_code, 5, -1,
                                           position_size, 1,
                                           ContextInfo)
                ...
                if g.short_position[g.current_stock_code] == 0:
                    ...
                    order_info = passorder(3, 1101, ContextInfo.account_id, g.current_stock_code, 5, -1,
                                           position_size, 1,
                                           ContextInfo)
                    log_info(f"  [交易执行] 下单结果: {order_info}")
        ...
        # 平仓操作
            ...
            if position_type > 0 and g.long_position[g.current_stock_code] == 1:  # 平多仓
                # 7 平多, 优先平昨
                # 修正：使用手数而不是股数进行平仓
                long_volume_in_lots = g.long_volume[g.current_stock_code] / contract_multiplier
                log_info(f"  [交易执行] 执行买入平仓操作: {long_volume_in_lots} 手，价格: {current_price:.4f}")
                log_info(
                    f"  [交易执行] 下单参数: 买入平仓, 限价单, 对手价, 市价, {long_volume_in_lots} 手 , {g.long_volume[g.current_stock_code]} 股")
                order_info = passorder(7, 1101, ContextInfo.account_id, g.current_stock_code, 5, -1,
                ...
                # 9 平空, 优先平昨
                # 修正：使用手数而不是股数进行平仓
                short_volume_in_lots = g.short_volume[g.current_stock_code] / contract_multiplier
                log_info(f"  [交易执行] 执行卖出平仓操作: {short_volume_in_lots} 手，价格: {current_price:.4f}")
                log_info(
                    f"  [交易执行] 下单参数: 卖出平仓, 限价单, 对手价, 市价, {short_volume_in_lots} 手 , {g.short_volume[g.current_stock_code]} 股")
                order_info = passorder(9, 1101, ContextInfo.account_id, g.current_stock_code, 5, -1,
