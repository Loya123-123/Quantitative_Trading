...
def init(ContextInfo):
    ...
    g.max_price_diff_ratio = 0.02  # 最大价差比例限制(2%)
    ...
    log_info(f"        最大价差比例: {g.max_price_diff_ratio}")
    ...
    g.last_main_contract_check = None  # 上次主力合约检查日期
    g.current_trading_contract = ContextInfo.stock_code  # 当前交易的主力合约
    ...
    log_info(f"        当前交易合约: {g.current_trading_contract}")
    ...
    # 设置每日定时检查主力合约（在流动性较好的时段）
    ContextInfo.run_time("check_main_contract_switch", "1nDay", "2019-10-14 10:00:00")
    log_info(f"[初始化] 设置每日10:00定时检查主力合约切换")
...
def handlebar(ContextInfo):
    ...
    # 检查是否需要切换主力合约（每日检查一次）
    check_main_contract_switch(ContextInfo)

    # 使用当前交易的主力合约进行后续操作
    current_contract = g.current_trading_contract
    log_info(f"[合约信息] 当前交易合约: {current_contract}")
    ...
    # 获取合约基础信息
    g.stock_contract_info = ContextInfo.get_instrument_detail(current_contract)
    ...
    try:
        ...
        price_data = get_price_data(ContextInfo, current_contract)
        log_info(f'[数据获取]: \n {str(price_data)}')
        ...
        account_info = get_account_info(ContextInfo, current_contract)
        ...
        signal = generate_signal(ContextInfo, price_data, current_contract)
        log_info(f"[信号生成] 生成的交易信号: {signal}")

        # 执行买卖操作
        if signal != (0, 0):
            log_info("[交易执行] 检测到交易信号，开始执行交易...")
            execute_trade(ContextInfo, signal, price_data, current_contract)
        else:
...
def check_main_contract_switch(ContextInfo):
    """
    检查主力合约切换
    通过run_time函数每日定时执行，在流动性较好的时段进行
    """
    try:
        current_date = datetime.strptime(g.current_date[:8], '%Y%m%d').date()

        # 检查是否已经检查过今天的主力合约
        if g.last_main_contract_check == current_date:
            return

        # 获取当前主力合约
        new_main_contract = ContextInfo.get_main_contract(ContextInfo.stock_code, g.current_date[:8])

        if not new_main_contract:
            log_info("[主力合约] 无法获取当前主力合约")
            return

        old_contract = g.current_trading_contract

        # 如果主力合约发生变化
        if new_main_contract != old_contract:
            log_info(f"[主力合约] 检测到主力合约变更: {old_contract} -> {new_main_contract}")

            # 获取两个合约的价格用于价差计算
            old_price_data = ContextInfo.get_market_data_ex(
                ['close'], [old_contract], period='1d', count=1,
                dividend_type=ContextInfo.dividend_type
            )

            new_price_data = ContextInfo.get_market_data_ex(
                ['close'], [new_main_contract], period='1d', count=1,
                dividend_type=ContextInfo.dividend_type
            )

            if (old_price_data and new_price_data and
                    old_contract in old_price_data and new_main_contract in new_price_data):

                old_price = old_price_data[old_contract]['close'].iloc[-1]
                new_price = new_price_data[new_main_contract]['close'].iloc[-1]

                # 计算价差比例
                price_diff_ratio = abs(old_price - new_price) / old_price

                log_info(f"[主力合约] 旧合约价格: {old_price}, 新合约价格: {new_price}")
                log_info(f"[主力合约] 价差比例: {price_diff_ratio:.4f}")

                # 如果价差超过限制，不进行切换
                if price_diff_ratio > g.max_price_diff_ratio:
                    log_info(f"[主力合约] 价差过大({price_diff_ratio:.4f} > {g.max_price_diff_ratio}), 暂不切换")
                    return

                # 执行主力合约切换
                log_info(f"[主力合约] 执行主力合约切换: {old_contract} -> {new_main_contract}")
                g.current_trading_contract = new_main_contract

                # 记录切换日志
                log_info(f"[主力合约] 主力合约切换完成，新的交易合约为: {new_main_contract}")

                # 如果有持仓，需要处理持仓转移（在实际交易中可能需要平仓再重新开仓）
                if g.long_position == 1 or g.short_position == 1:
                    log_info(f"[主力合约] 检测到持仓，需要注意合约切换对持仓的影响")
            else:
                log_info("[主力合约] 无法获取合约价格数据进行价差计算")
        else:
            log_info(f"[主力合约] 主力合约未发生变化: {old_contract}")

        # 更新上次检查日期
        g.last_main_contract_check = current_date

    except Exception as e:
        log_info(f"[主力合约] 检查主力合约切换时发生错误: {e}")


def get_price_data(ContextInfo, contract_code):
    """
    获取计算需要的数据
    ...
    try:
        ...
        if not history_market_data or contract_code not in history_market_data:
            log_info("  [价格数据] 获取历史市场数据为空")
            return None

        history_df = history_market_data[contract_code]

        # 将时间戳转换为可读的时间格式
        history_df['time'] = history_df['time'].apply(lambda x: timetag_to_datetime(x, '%Y-%m-%d %H:%M:%S'))

        log_info(
            f"  [价格数据] 请求参数 - 标的: {contract_code}, 周期: {ContextInfo.period}, 数量: 1")
        ...
        #     subscribe=True
        # )
        # log_info(f" [价格数据] : 当天最新数据： {current_market_data}")
        # current_df = current_market_data[contract_code]
        # if 'time' in current_df.columns:
        #     current_df = current_df.copy()  # 避免修改原始数据
        #     current_df['time'] = current_df['time'].apply(lambda x: timetag_to_datetime(x, '%Y-%m-%d %H:%M:%S'))
        ...
        if not current_market_data_more or contract_code not in current_market_data_more:
            log_info("  [价格数据] 获取当日市场数据为空")
            return None

        current_df_more = current_market_data_more[contract_code]
...
def get_account_info(ContextInfo, contract_code):
    """
    获取账户信息
    包括可用资金、总权益、持仓等
    """
...
def generate_signal(ContextInfo, price_data, contract_code):
    """
    决策分区 - 生成交易信号
    ...
    try:
        ...
        if g.long_position == 1:  # 多头已有持仓
            ...
            long_history_market_data_max = long_history_market_data[contract_code]['high'].iloc[:-1].max()
            # 历史最高价 和 当日最高价比较
            ...
            # g.highest_after_entry = max(long_history_market_data[contract_code]['high'].iloc[:-1].max(), current_high)
        ...
        if g.short_position == 1:  # 空头已有持仓
            ...
            short_history_market_data_min = short_history_market_data[contract_code]['low'].iloc[:-1].min()
            # 历史最低价 和 当日最低价比较
...
def execute_trade(ContextInfo, signal, price_data, contract_code):
    """
    执行买卖操作
    根据交易信号执行具体的下单操作
    ...
    try:
        ...
        contract_multiplier = ContextInfo.get_contract_multiplier(contract_code)
        ...
        if signal_type > 0:  # 开仓信号
            if position_type > 0:  # 做多
                ...
                if g.long_position == 0:
                    ...
                    order_info = passorder(0, 1101, ContextInfo.account_id, contract_code, 5, -1,
                                           position_size, 1,
                                           ContextInfo)
                ...
                if g.short_position == 0:
                    ...
                    order_info = passorder(3, 1101, ContextInfo.account_id, contract_code, 5, -1,
                                           position_size, 1,
                                           ContextInfo)
                    log_info(f"  [交易执行] 下单结果: {order_info}")
                    g.short_position = 1
        ...
        # 平仓操作
            ...
            if position_type > 0 and g.long_position == 1:  # 平多仓
                ...
                log_info(f"  [交易执行] 下单参数: 买入平仓, 限价单, 对手价, 市价, {abs(ContextInfo.long_volume)} 股")
                order_info = passorder(7, 1101, ContextInfo.account_id, contract_code, 5, -1,
                ...
                # 9 平空, 优先平昨
                log_info(f"  [交易执行] 下单参数: 卖出平仓, 限价单, 对手价, 市价, {ContextInfo.short_volume} 股")
                order_info = passorder(9, 1101, ContextInfo.account_id, contract_code, 5, -1,
