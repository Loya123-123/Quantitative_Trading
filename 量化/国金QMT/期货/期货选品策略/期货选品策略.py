    ...
    try:
        ...
        for idx, row in pools_df.iterrows():
            ...
            try:
                ...
                # 昨日收盘价 = history_df['close'].iloc[0]（最近一天）
                # 11日前收盘价 = history_df['close'].iloc[11]（第11天前）
                close_yesterday = history_df['close'].iloc[0]  # 昨日收盘价（第0根K线）
                close_11days_ago = history_df['close'].iloc[11]  # 11日前收盘价（第11根K线）
                ...
                volatility_sum = 0
                for i in range(1, 11):  # 第1根到第10根K线
