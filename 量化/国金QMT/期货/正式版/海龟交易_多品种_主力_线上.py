        ...
        if contract in g.last_open_order:
            ...
            if time_diff < g.order_cooldown_seconds:
                ...
                if contract in g.position_code:
                    # 持仓已确认，可以处理
                    log_info(f"  [冷却过滤] 合约 {contract} 已过冷却期且持仓已确认，方向: {last_order['direction']}, 已过时间: {time_diff:.1f}秒，允许处理")
                    # 清理记录，释放冷却
                    del g.last_open_order[contract]
                else:
                    # 持仓未确认，在待处理列表中剔除
                    log_info(f"  [冷却过滤] 合约 {contract} 处于开仓冷却期，方向: {last_order['direction']}, 已过时间: {time_diff:.1f}秒，持仓未确认，剔除本次处理")
                    continue
            else:
                # 已过冷却期，清理记录
