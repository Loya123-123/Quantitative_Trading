...
买入头寸：按照ContextInfo.stock_codes_dict中设定的目标手数分批买入，每次买入目标手数的三分之一，直到达到目标手数
加仓规则：可以分批达到目标手数，做多和做空可以同时存在，互不影响。

如果是开仓信号且持仓不满4个 或 合约距离到期小于30天 则才能开仓。

策略核心变量说明：
- g.entry_window: 入市通道周期（突破周期），默认10天
- g.exit_window: 止盈通道周期，默认4天
- g.atr_window: ATR计算周期，默认10天
- g.stop_profit_ratio: 止盈比例，默认0.2（20%）
- g.stop_loss_multiplier: 止损ATR倍数，默认1
- g.position_limit: 持仓上限，默认5个
- g.near_expiry_days: 临近到期日天数上限，默认30天
- g.position_size: 每个合约的目标手数，从ContextInfo.stock_codes_dict继承
- g.current_long_volume/g.current_short_volume: 当前实际持仓手数，用于跟踪是否达到目标手数
- g.pending_cancel_contracts: 需要撤销委托的合约列表，用于避免对正在撤销的合约进行操作

代码逻辑流程：
1. 初始化阶段：设置策略参数、交易标的、全局变量
2. 定时执行(run_time_handlebar)：
   a. 获取主力合约替代连续合约
   b. 获取账户信息（包括资金、持仓、未成交委托）
   c. 处理未成交委托（撤销未成交的委托，并将合约加入pending_cancel_contracts列表）
   d. 合并交易合约列表和持仓合约列表
   e. 从交易合约中剔除需要撤销委托的合约（避免对正在撤销的合约进行策略逻辑处理）
   f. 遍历每个合约执行策略逻辑
   g. 获取合约基础信息和到期日
   h. 计算ATR和N值
   i. 生成交易信号
...
交易信号生成逻辑：
- 多头开仓：无持仓或未达目标手数，价格突破入市上轨，且合约未临近到期，且总持仓未超限
- 多头平仓：有持仓，价格跌破离市下轨且回撤达到阈值 或 价格跌破止损线
- 空头开仓：无持仓或未达目标手数，价格跌破入市下轨，且合约未临近到期，且总持仓未超限
- 空头平仓：有持仓，价格突破离市上轨且回撤达到阈值 或 价格上涨超止损线

交易执行逻辑：
- 开仓：每次买入目标手数的三分之一，直到达到目标手数
- 平仓：一次性平掉全部持仓
- 风控：检查合约到期日、持仓限制等条件

风控措施：
- 对于需要撤销委托的合约，不会执行任何策略逻辑，避免重复操作
- 检查合约到期日，避免对即将到期的合约开仓
- 控制持仓数量，不超过设定上限
"""
# coding:gbk
...
def init(ContextInfo):
    ...
    g.current_long_volume = {}  # 当前多头实际持仓手数，为每个合约保存
    g.current_short_volume = {}  # 当前空头实际持仓手数，为每个合约保存
...
def run_time_handlebar(ContextInfo):  # 定时运行
    ...
    处理流程：
    1. 检查是否为最后一个K线周期及交易时间
    2. 获取主力合约替代连续合约
    3. 获取账户信息（资金、持仓、未成交委托）
    4. 撤销未成交委托，并记录到pending_cancel_contracts列表
    5. 合并交易合约列表和持仓合约列表
    6. 从交易合约中剔除需要撤销委托的合约（避免对正在撤销的合约进行策略逻辑处理）
    7. 遍历剩余合约执行策略逻辑
    """
    ...
    try:
        ...
        if order_details:
            ...
            for order in order_details:
                ...
                log_info(f"  [账户信息] 获取委托记录: {symbol} {order_status}")
        ...
        if position_details:
            ...
            if not aggregated_positions.empty:
                ...
                for _, row in aggregated_positions.iterrows():
                    ...
                    g.current_long_volume[symbol] = 0  # 重置当前多头实际持仓手数
                    g.current_short_volume[symbol] = 0  # 重置当前空头实际持仓手数
                    ...
                    if position_type == 48:  # 多头持仓
                        ...
                        g.current_long_volume[symbol] = volume  # 当前多头实际持仓手数
                        ...
                        g.current_short_volume[symbol] = volume  # 当前空头实际持仓手数
...
def generate_signal(ContextInfo, price_data):
    ...
    try:
        ...
        # 海龟交易法则信号判断
        # 判断多头信号（无多头持仓时可做多，或多头持仓但未达目标手数时可继续做多）
        # 检查当前多头持仓手数是否小于目标手数
        current_long_vol = g.current_long_volume.get(g.current_stock_code, 0)
        target_size = g.position_size[g.current_stock_code]
        can_add_long = current_long_vol < target_size

        if (g.long_position[g.current_stock_code] == 0 or can_add_long) and g.expire_date_diff[
            g.current_stock_code] > g.near_expiry_days and g.position_count < g.position_limit:  # 无多头持仓或未达目标手数，判断是否做多 距离到期日大于30天 持仓小于4个
            # 买入1（做多）：当日价格 > 前10日收盘价最高点时，即当前价格突破前10日高点，当日立刻执行做多
        ...
        # 判断空头信号（无空头持仓时可做空，或空头持仓但未达目标手数时可继续做空）
        # 检查当前空头持仓手数是否小于目标手数
        current_short_vol = g.current_short_volume.get(g.current_stock_code, 0)
        can_add_short = current_short_vol < target_size

        if (g.short_position[g.current_stock_code] == 0 or can_add_short) and g.expire_date_diff[
            g.current_stock_code] > g.near_expiry_days and g.position_count < g.position_limit:  # 无空头持仓或未达目标手数，判断是否做空 且 距离到期日大于30天 持仓小于4个
...
def execute_trade(ContextInfo, signal, price_data):
    ...
    try:
        ...
        # 计算目标手数的三分之一
        target_third_size = int(g.position_size[g.current_stock_code] / 3)
        if target_third_size < 1:
            target_third_size = 1  # 至少买1手
        ...
        if signal_type > 0:  # 开仓信号
            if position_type > 0:  # 做多
                ...
                if current_long_vol < target_size:
                    # 计算还需要买入的手数
                    remaining_long_vol = target_size - current_long_vol
                    # 本次买入手数不超过剩余手数和目标手数的三分之一
                    actual_buy_vol = min(target_third_size, remaining_long_vol)

                    log_info(
                        f"  [交易执行] 执行买入开仓操作 下单参数: 买入开仓, 对手价, 价格: {current_price:.4f}, {actual_buy_vol}手数 (目标{target_size}手, 已持{current_long_vol}手)")
                    # passorder( opType, orderType, accountid , orderCode, prType, price, volume , strategyName, quickTrade, userOrderId , ContextInfo)
                    #        #  操作号    组合方式     资金账号    品种代码     报价类型  价格    下单量    策略名称        忽略下单标记  投资备注        策略上下文
                    ...
                    g.current_long_volume[g.current_stock_code] = current_long_vol + actual_buy_vol
                    g.long_volume[g.current_stock_code] = current_long_vol + actual_buy_vol

                    # 只有当达到目标手数时才增加持仓计数
                    if current_long_vol == 0:  # 首次买入，增加持仓计数
                        g.position_count = g.position_count + 1
                        log_info(f"  [交易执行] 持仓计数器: {g.position_count}")
                else:
                    log_info(f"  [交易执行] 多头持仓已达目标手数 {target_size}，不继续开仓")
                ...
                if current_short_vol < target_size:
                    # 计算还需要买入的手数
                    remaining_short_vol = target_size - current_short_vol
                    # 本次买入手数不超过剩余手数和目标手数的三分之一
                    actual_sell_vol = min(target_third_size, remaining_short_vol)
                    message = f"  [交易执行] 执行卖出开仓操作下单参数: 卖出开仓, 限价单, 对手价, 市价, {actual_sell_vol} 手数，价格: {current_price:.4f} (目标{target_size}手, 已持{current_short_vol}手)"
                    log_info(message)
                    send_feishu_message(message)
                    ...
                    g.current_short_volume[g.current_stock_code] = current_short_vol + actual_sell_vol
                    g.short_volume[g.current_stock_code] = current_short_vol + actual_sell_vol

                    # 只有当达到目标手数时才增加持仓计数
                    if current_short_vol == 0:  # 首次买入，增加持仓计数
                        g.position_count = g.position_count + 1
                        log_info(f"  [交易执行] 持仓计数器: {g.position_count}")
                else:
                    log_info(f"  [交易执行] 空头持仓已达目标手数 {target_size}，不继续开仓")
        ...
        # 平仓操作
            ...
            if position_type > 0 and g.long_position[g.current_stock_code] == 1:  # 平多仓
                ...
                log_info(
                    f"  [交易执行] 执行买入平仓操作：下单参数: 买入平仓, 对手价,  {current_long_vol} 手持仓，价格: {current_price:.4f} ")

                order_info = passorder(7, 1101, ContextInfo.account_id, g.current_stock_code, 14, -1,
                                       current_long_vol, 1, ContextInfo)
                log_info(f"  [交易执行] 下单结果: {order_info}")
                ...
                g.current_long_volume[g.current_stock_code] = 0
                g.long_volume[g.current_stock_code] = 0
                ...
                message = f"  [交易执行] 执行卖出平仓操作:下单参数: 卖出平仓, 对手价,, {current_short_vol} 手持仓 ，价格: {current_price:.4f}"
                log_info(message)
                ...
                g.current_short_volume[g.current_stock_code] = 0
                g.short_volume[g.current_stock_code] = 0
...
class G():
    def __init__(self):
        # 策略参数
        self.entry_window = None  # 入市通道周期（突破周期）
        self.exit_window = None  # 止盈通道周期
        self.atr_window = None  # ATR计算周期
        self.stop_profit_ratio = None  # 止盈比例
        self.stop_loss_multiplier = None  # 止损ATR倍数
        self.position_limit = None  # 持仓上限
        self.near_expiry_days = None  # 临近到期日天数上限
        self.capital_rate = None  # 资金比例

        # 策略状态变量
        self.N = {}  # 波动幅度(N值/ATR)，为每个合约保存
        self.long_position = {}  # 多头持仓：0-无仓位，1-持有多头，为每个合约保存
        self.short_position = {}  # 空头持仓：0-无仓位，1-持有空头，为每个合约保存
        self.highest_after_entry = {}  # 入市后的最高价，为每个合约保存
        self.lowest_after_entry = {}  # 入市后的最低价，为每个合约保存
        self.long_open_date = {}  # 多头开仓日期，为每个合约保存
        self.short_open_date = {}  # 空头开仓日期，为每个合约保存
        self.long_volume = {}  # 多头持仓量，为每个合约保存
        self.long_entry_price = {}  # 多头持仓价，为每个合约保存
        self.short_volume = {}  # 空头持仓量，为每个合约保存
        self.short_entry_price = {}  # 空头持仓价，为每个合约保存

        self.expire_date = {}  # 当前需要交易的合约的到期日，为每个合约保存
        self.current_trading_contracts = []  # 当前需要交易的合约，为每个合约保存
        self.expire_date_diff = {}  # 到期日与当前日期的差值，为每个合约保存
        self.position_count = 0  # 当前持仓数量
        self.position_code = []  # 当前持仓的合约
        self.position_size = {}  # 交易合约对应的手数，为每个合约保存
        self.pending_cancel_contracts = []  # 需要撤销委托的合约列表
        g.current_long_volume = {}  # 当前多头实际持仓手数，为每个合约保存
        g.current_short_volume = {}  # 当前空头实际持仓手数，为每个合约保存

        # 其他变量
        self.current_date_bar = None  # 当前K线时间
        self.current_date_bar_hour = None  # 当前K线小时
        self.current_stock_code = None  # 当前处理的股票代码
        self.long_margin_ratio = None  # 多头保证金率
        self.short_margin_ratio = None  # 空头保证金率
