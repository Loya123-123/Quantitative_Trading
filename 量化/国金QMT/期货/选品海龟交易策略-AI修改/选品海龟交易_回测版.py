...
def init(ContextInfo):
    ...
    # 账户信息
    ContextInfo.account_id = account  # 回测/实盘都使用account变量
    ...
    # 选品
    import os
g.work_dir = os.path.dirname(__file__) + '\\'  # 使用脚本所在目录
    # 品种池文件路径
    ...
    try:
        ...
        if signal_type > 0:  # 开仓信号
            if position_type > 0:  # 做多
                ...
                if g.long_position[g.current_stock_code] == 0:
                    ...
                    remark = f"海龟买入_{g.current_stock_code}_{datetime.now().strftime('%H%M%S')}"
                    order_info = passorder(33, 1101, ContextInfo.account_id, g.current_stock_code, 
                                           g.position_size[g.current_stock_code], -1, 14,
                                           "海龟策略", remark, ContextInfo)
                ...
                if g.short_position[g.current_stock_code] == 0:
                    # 3: 开空
                    remark = f"海龟卖空_{g.current_stock_code}_{datetime.now().strftime('%H%M%S')}"
                    order_info = passorder(34, 1101, ContextInfo.account_id, g.current_stock_code,
                                           g.position_size[g.current_stock_code], -1, 14,
                                           "海龟策略", remark, ContextInfo)
                    log_info(f"  [交易执行] 下单结果: {order_info}")
                    if order_info == 0:
        ...
        # 平仓操作
            ...
            if position_type > 0 and g.long_position[g.current_stock_code] == 1:  # 平多仓
                ...
                remark = f"海龟平多_{g.current_stock_code}_{datetime.now().strftime('%H%M%S')}"
                    order_info = passorder(34, 1101, ContextInfo.account_id, g.current_stock_code,
                                       g.long_volume[g.current_stock_code], -1, 14,
                                       "海龟策略", remark, ContextInfo)
                log_info(f"  [交易执行] 下单结果: {order_info}")
                if order_info == 0:
                ...
                remark = f"海龟平空_{g.current_stock_code}_{datetime.now().strftime('%H%M%S')}"
                    order_info = passorder(33, 1101, ContextInfo.account_id, g.current_stock_code,
                                       g.short_volume[g.current_stock_code], -1, 14,
                                       "海龟策略", remark, ContextInfo)
                log_info(f"  [交易执行] 下单结果: {order_info}")
                if order_info == 0:
